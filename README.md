...
**This models a generic organizational concept — a human authorizing deployment after quality gates pass — not the specific technical implementation of any real company.** After `Main Sanity Tests` passes on `main`, the pipeline pauses at a `Deployment Authorization` stage and waits for an explicit decision in the Jenkins UI (button: **"Authorize Simulated Deployment"**). The prompt is deliberately worded so it cannot be read as claiming a real SIT/UAT environment is about to be contacted.
...
- **Approver identity, when captured, comes from Jenkins' own authenticated UI session** (`submitterParameter`) — not from any credential, token, or secret. If that mechanism doesn't return a usable value, `approvedBy` is recorded as `null` rather than guessed.
...
- **Known limitation:** distinguishing an explicit UI rejection from an unrelated administrative build abort relies on best-effort exception inspection, since Jenkins does not expose a version-stable, fully qualified way to tell them apart. In practice this repository's realistic operating model (a single local operator) makes that edge case unlikely, but it is a disclosed limitation, not a silent one.
