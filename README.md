...
flowchart TD
    ...
    subgraph MAIN["main (post-merge)"]
        ...
        C5 -->|rejected| C9["Release Evidence"]
        C5 -->|timeout / external Abort| C10["Jenkins ABORTED (no Release Evidence)"]
        C8 --> C9
    end
```
...
- **Deployment Authorization: approval timeout or external Jenkins Abort** → the pipeline intentionally does **not** distinguish these two inside sandboxed Groovy (see "Manual deployment authorization" below for why). Both simply propagate as Jenkins' own native interruption handling — the build finishes `ABORTED`. None of the simulated SIT/UAT stages execute, and — unlike an explicit rejection — **no `deployment-authorization.json` is written and Release Evidence is not guaranteed to run** for an interrupted build, exactly as an interrupted `Main Sanity` build today produces no evidence either.
...
- **The business decision is an explicit `DECISION` parameter value, not the Jenkins input step's own Abort action.** Clicking Jenkins' generic "Abort" link on the input prompt is **not** how a deployment is rejected in this pipeline — rejection is chosen the same way approval is, by selecting `REJECT` and submitting normally. This means `REJECT` returns from the `input` step exactly like `APPROVE` does — no exception is thrown or caught to represent a business decision, on either path.
- **The pipeline intentionally does not distinguish an approval timeout from an external Jenkins Abort inside sandboxed Groovy.** An earlier implementation attempted to positively identify the authorization timeout by inspecting the interruption's structured cause (`FlowInterruptedException.getCauses()`), but this Jenkins instance does not have that method approved in Script Security, and a runtime test confirmed it — `RejectedAccessException: Scripts not permitted to use method ... FlowInterruptedException getCauses`. Rather than approve that signature (which would require Jenkins administrator action, deliberately out of scope for a repository-only fix) or weaken Script Security, this is now a **deliberate architecture decision**: `timeout(24 HOURS){ input(...) }` is called with no surrounding `try`/`catch` at all. If the timeout elapses, or the build is aborted externally, the interruption is never caught by this pipeline's own code — it propagates exactly as Jenkins would handle it for any other stage, and the build simply finishes `ABORTED`. No sandbox-restricted method is ever called, because no code here touches the interruption object in any way.
- **Rejection is not a test failure** — see the failure-behavior bullets above for exactly how it is reported, and how it differs from an approval timeout or external Abort (both of which are plain Jenkins interruptions, not business outcomes).
